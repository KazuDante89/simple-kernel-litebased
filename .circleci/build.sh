...
git clone --depth=1 https://github.com/KazuDante89/simple-kernel-litebased -b lite kernel
cd kernel
...
export ARCH=arm64
export KBUILD_BUILD_HOST=simple-kernel
...
function zipping() {
    cd AnyKernel || exit 1
    zip -r9 Simple-Kernel-litebeta2-${TANGGAL}.zip *
    cd .. 
}
sticker
sendinfo
